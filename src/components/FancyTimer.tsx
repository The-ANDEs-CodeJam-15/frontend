...
const FancyTimer: React.FC<FancyTimerProps> = ({ timeRemaining, totalTime, currentRound, totalRounds }) => {
  ...
  const displayPercentage = (Math.max(0, displayTime - 1) / (safeTotalTime)) * 100;
  ...
  return (
    <div className="flex flex-col items-center py-8">
      <div className="relative">
        ...
        />
        <h1 className="text-center text-xl font-bold text-white">{`Round ${currentRound} of ${totalRounds}`}</h1>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <div
              className={`text-5xl font-bold tabular-nums text-center mt-6 transition-colors duration-300 ${isUrgent ? "text-red-500 animate-pulse" : "text-neutral-50" }`}>
              {displayTime}
            </div>
          </div>
        </div>
